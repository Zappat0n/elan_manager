...
public class Pdf_EoY_Reports extends PDFForm_Reports {
    ...
    public Pdf_EoY_Reports(Connection co, Integer studentId, Integer classroom, Date reportDate, BufferedImage logo,
                           ReportManager reportManager, Boolean addCommentsSection, Boolean includePictures,
                           DefaultListModel<String> log) {
        super(co, studentId, classroom, reportDate, "EoY", logo);
        try {
            this.log = log;

            eoyManager = new EoYManager(reportManager, null, new java.sql.Date(reportDate.getTime()), classroom, studentId);
    ...
    private void addPictures() {
        try {
            ...
            MediaManager manager = new MediaManager(ApplicationLoader.cacheManager,ApplicationLoader.settingsManager,
                    ApplicationLoader.bdManager, studentId, new java.sql.Date(date1.getTime()),
                    new java.sql.Date(date2.getTime()), doc, font, margin, fileName, log);
            manager.execute();
        }
        catch (Exception e) {
            e.printStackTrace();
    ...
    private void addPhrase(PDPage page, int x, int y) throws IOException {
        ...
        contents.showText(ApplicationLoader.settingsManager.language == 0 ? "Final interview" : "Entrevista Final");
        contents.newLineAtOffset(0, -leading * 1.5f);
        contents.showText(ApplicationLoader.cacheManager.getNameStageOfClassroom(classroom));
        contents.endText();
        contents.close();
    }
    ...
    private int addTopTable() throws IOException {
        ...
        addCellToRowBuilder(rowBuilder, LanguageManager.NAME[ApplicationLoader.settingsManager.language] + ":" +
                ApplicationLoader.cacheManager.students.get(studentId)[0]);
        addCellToRowBuilder(rowBuilder, LanguageManager.DATE[ApplicationLoader.settingsManager.language] + ":" +
        ...
        addCellToRowBuilder(rowBuilder, LanguageManager.AGE[ApplicationLoader.settingsManager.language] + ":" +
                ApplicationLoader.cacheManager.getChildrenAge(studentId));
        addCellToRowBuilder(rowBuilder, LanguageManager.GUIDE[ApplicationLoader.settingsManager.language] + ":" +
                ApplicationLoader.cacheManager.teachers.get(eoyManager.teacher)[0]);
        tableBuilder.addRow(rowBuilder.build());
    ...
    private int addMiddleTable() throws IOException {
        ...
        Row.RowBuilder rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.PSED[ApplicationLoader.settingsManager.language]);
        addCellToRowBuilder(rowBuilder, data.get(0) != null ? data.get(0).replace("\t", "") : "");
        tableBuilder.addRow(rowBuilder.build());

        rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.WORKINITIATIVE[ApplicationLoader.settingsManager.language]);
        addCellToRowBuilder(rowBuilder, data.get(1) != null ? data.get(1).replace("\t", "") : "");
        tableBuilder.addRow(rowBuilder.build());

        rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.FREEDOM[ApplicationLoader.settingsManager.language]);
        addCellToRowBuilder(rowBuilder, data.get(2) != null ? data.get(2).replace("\t", "") : "");
        tableBuilder.addRow(rowBuilder.build());

        rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.PATTERNS[ApplicationLoader.settingsManager.language]);
        addCellToRowBuilder(rowBuilder, data.get(3) != null ? data.get(3).replace("\t", "") : "");
        tableBuilder.addRow(rowBuilder.build());

        rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.CHALLENGES[ApplicationLoader.settingsManager.language]);
        addCellToRowBuilder(rowBuilder, data.get(4) != null ? data.get(4).replace("\t", "") : "");
        tableBuilder.addRow(rowBuilder.build());

        rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.BEHAVIOUR[ApplicationLoader.settingsManager.language]);
        addCellToRowBuilder(rowBuilder, data.get(5) != null ? data.get(5).replace("\t", "") : "");
        tableBuilder.addRow(rowBuilder.build());

        rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.CONCENTRATION[ApplicationLoader.settingsManager.language]);
        addCellToRowBuilder(rowBuilder, data.get(6) != null ? data.get(6).replace("\t", "") : "");
        tableBuilder.addRow(rowBuilder.build());

        rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.SELF[ApplicationLoader.settingsManager.language]);
        addCellToRowBuilder(rowBuilder, data.get(7) != null ? data.get(7).replace("\t", "") : "");
        tableBuilder.addRow(rowBuilder.build());

        rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.RELATIONSHIPS[ApplicationLoader.settingsManager.language]);
        addCellToRowBuilder(rowBuilder, data.get(8) != null ? data.get(8).replace("\t", "") : "");
        tableBuilder.addRow(rowBuilder.build());

        rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.LIMITS[ApplicationLoader.settingsManager.language]);
        addCellToRowBuilder(rowBuilder, data.get(9) != null ? data.get(9).replace("\t", "") : "");
        tableBuilder.addRow(rowBuilder.build());

        rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.MOVES[ApplicationLoader.settingsManager.language]);
    ...
    private int addBottomTable() throws IOException {
        ...
        Row.RowBuilder rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.LITERACY[ApplicationLoader.settingsManager.language]);
        addCellToRowBuilder(rowBuilder, data.get(11) != null ? data.get(11).replace("\t", "") : "");
        tableBuilder.addRow(rowBuilder.build());

        rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.MATHS[ApplicationLoader.settingsManager.language]);
        addCellToRowBuilder(rowBuilder, data.get(12) != null ? data.get(12).replace("\t", "") : "");
        tableBuilder.addRow(rowBuilder.build());

        rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.UNDERSTANDING[ApplicationLoader.settingsManager.language]);
        addCellToRowBuilder(rowBuilder, data.get(13) != null ? data.get(13).replace("\t", "") : "");
        tableBuilder.addRow(rowBuilder.build());

        rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.PHYSICAL_DEV[ApplicationLoader.settingsManager.language]);
        addCellToRowBuilder(rowBuilder, data.get(14) != null ? data.get(14).replace("\t", "") : "");
        tableBuilder.addRow(rowBuilder.build());

        rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.ARTS[ApplicationLoader.settingsManager.language]);
    ...
    private Integer addComments() throws IOException {
        ...
        Row.RowBuilder rowBuilder = new Row.RowBuilder();
        addCellToRowBuilder(rowBuilder, LanguageManager.COMMENTS[ApplicationLoader.settingsManager.language]);
        tableBuilder.addRow(rowBuilder.build());
    ...
    private String getTeachers() {
        ...
        MySet set = ApplicationLoader.bdManager.getValues(co, BDManager.tableTeachers, TableTeachers.classroom + "=" + classroom);
        while (set.next()) {
