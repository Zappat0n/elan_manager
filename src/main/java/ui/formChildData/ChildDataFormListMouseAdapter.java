 ...
 */
public class ChildDataFormListMouseAdapter extends MouseAdapter {
    private final ChildDataForm form;
    final ArrayList<ChildDataFormListItem> subs = new ArrayList<>();


    public ChildDataFormListMouseAdapter(ChildDataForm form) {
        this.form = form;
    }
    ...
    private void checkIndex(int index, ChildDataFormListItem item) {
        Boolean add = false;
        switch (index) {
            case 0 : add = !item.getBox1();
                     break;
            case 1 : add = !item.getBox2();
                break;
            case 2 : add = !item.getBox3();
        }
